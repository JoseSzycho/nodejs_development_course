const { observeObject } = require("../tasks/task5/observeObject");
...
test("Expect called properties of task 1 object to be logged when accessed", () => {
  ...
  const proxyPerson = observeObject(person, logFunction);
...
test("Expect observedObject to store and give values correctly", () => {
  ...
  const proxyStudent = observeObject(student, doNothing);
